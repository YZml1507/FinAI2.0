...
| G-DOC-1 | `doc_paths`（缺省扫 `_REPO_ROOT/docs`）, truth artifact, `expected_gate_count`, `expected_test_baseline` | docs↔产物指标一致性声明绑定本仓文档约定与 `TEST_BASELINE_PASSED` SSOT | 🏠 | PASS⚠（扫的是宿主仓文档） |
