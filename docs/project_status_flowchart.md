...
flowchart LR
  ...
  P1 --> G2[G2 三源验收 ✅]
  G2 --> P2C[Phase 2 回测引擎 ⏵当前<br/>T201 ✅ T202 ✅ T203 ✅<br/>T204–T207 待启动]
...
> ✅ **Phase 1 全清零（2026-08-31）**：T105（`8282cd4`，62 绿）/ T106+T107（`5e08574`，127 绿）/ T110（`2ffbf7b`，152 绿）/ T109（`c5d75bf`，162 绿）全部入库并勾选，G2 门禁通过。结构 = Parquet 落盘（`data/daily_bars/{symbol}/{year}.parquet`）+ `data/` 包（collector✅/cleaner✅/financial_pit✅/universe✅/incremental✅/acceptance✅ 全部入库）。**其后 Phase 2 回测引擎：T201 引擎核心 + T202 五必挂用例已入库**（2026-09-01，commit `8fca14f`，319 离线单测绿）；**T203 费用模型已入库**（2026-09-01，commit `edd8d2f`，352 离线单测绿：六科目分段费率逐项透视 + `make_fee_model` 注入撮合 + 07 号核对表逐项核）；T204–T207 待启动。

## Phase 2–6 路线（Phase 2 进行中）

```mermaid
flowchart LR
  P2[Phase 2 回测引擎 ⏵当前<br/>T201–T203 ✅ / T204–T207 / G3] --> P3[Phase 3 策略与组合<br/>T301–T305 / G4]
...
| 项目 | 结果 | 证据 |
|---|---|---|
| 代码仓 HEAD | ✅ `edd8d2f`（T203 费用模型入库）+ 本状态标记提交 | 本地 `git rev-parse HEAD` |
| 计划仓 HEAD | ✅ `05ba8cf`（tasks.md 勾 T203 + TK-9 修订日志） | 本地 `git rev-parse HEAD` |
| spec 快照一致 | ✅ `docs/spec/001-…/tasks.md` 与计划仓逐字节一致 | SHA-256 双端同值（2026-09-01） |
| 两仓已推送 | ✅ FinAI2.0（`8fca14f..edd8d2f` + 本提交）/ research-finai（`fd67351..05ba8cf`） | `git push origin master` 输出 |
...
| 离线单测 | ✅ **352 passed**（162 原有 + T201×140 + T202×17 + T203×33；2026-09-01 本窗复跑实证 exit 0） | `py -3.11 -m pytest tests/ …` |
...
| T204–T207 | ⬜ 待启动 | T204 成交模型（价格模型注入点 + 滑点入价 + 敏感度对比）/ T205 指标 / T206 registry / T207 门禁 G3 |
| 测试产物残留 | ✅ 无新增残留 | 测试走 tmp_path；审计临时目录用完即删 |
| research-finai 工作树 | ✅ 干净（`05ba8cf` 已提交） | `git status` |
