...
## 六、收单登记（2026-09-19 19:5x）★判负——双轴全劣，线索关闭

- **结果**：`d2-lowvol50`（27.6min，FINISHED）vs 对照 `isst-e8b`： <!-- gate-doc-ignore: 历史快照（消融实验实测对照值，非基线声明），⛔ 不改史 -->
  CAGR 3.6172% vs 8.5814% / MDD 26.6911% vs 17.3990% /
  换手 4.9170 vs 4.6074 / 110 笔 vs 156 笔 / fees 21.9k vs 22.3k /
  胜率 44.5% vs 44.2%。
- **裁决（§四 主判据，口径未动）**：ΔMDD=+9.29pp >0 且
  ΔCAGR=−4.96pp <−1.5pp ⇒ **判负**——低波翼未降回撤（反劣化）
  且收益代价远超红线，线索关闭。
- **归因线索（登记不改判）**：候选内截掉 vol 上半区后，留下的
  「低波高息」票在本池大概率是**价值陷阱型**（稳定阴跌/周期顶点
  的高息假象）——VOL 因子的负 alpha 方向在本池内反转。这与
  e1-veto −1.02pp、C3 池重选 +0.66pp/MDD+7.8pp 共同构成第三次
  「池内因子改造不传递」证据：e8b 的 top5-dv 排序本身已经是
  有效的截面选择器，叠加风控类筛选反而剔除 alpha 载体。
- **产物**：`experiments/lab/d2-lowvol50/`（含完整 run 产物）。
