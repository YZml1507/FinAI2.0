...
⏳ 状态：**已裁决——e8b 六维门禁 G-2 判负，不晋级**（2026-09-17，
±10% 扰动无平台区；G-1/G-3/G-5/G-6 已过，e8b 保留实验冠军身份）。
...
悬崖敏感即弃。扰动实验发起与分段验证设计属 L3。

### Q5【L3】扩池/换底层资产 ✅ 触发成立——当前主线

触发已满足（e8b CAGR<9% 判据命中，EXPANSION_MEMO §二已填表）。
**接续更新：R8 已交付，红利低波指数层暂停实施，不再等 Hermes。**
本地报告位置与接收记录见 TASK_TRACKER「当前接续」。先审计 R8 无前视
时序、参考层现金/残留亏损处理，核对 e8b 全档位现金计息和指数/ETF
数据口径，再决定机制研究方向。指数内部分散不能消除档位硬阈值，
既有 G-2 不得豁免。阈值机制研究与红利池重选均待归因，不自动立项。
⛔ 不许在 487 红利池内挖因子、不许重开已封顶路线。
