...
## 五、收单登记（2026-09-20 03:4x 全臂收齐）

| 臂 | CAGR | ΔCAGR | MDD | ΔMDD | 换手 | trips | 裁决 |
|---|---|---|---|---|---|---|---|
| e18-pos10 | 5.74% | −2.85pp | 17.32% | −0.08pp | 4.39 | 142 | 判负 | <!-- gate-doc-ignore: 历史快照（消融实验实测值，非基线声明），⛔ 不改史 -->
| e18-floor10 | 7.76% | −0.82pp | 18.83% | +1.43pp | 4.80 | 185 | 判负（地板本身即过滤器：放行尾部小额仓位反而摊薄） | <!-- gate-doc-ignore: 历史快照（消融实验实测值，非基线声明），⛔ 不改史 -->
| e18-pos15f10 | 3.76% | −4.82pp | 21.21% | +3.81pp | 4.73 | 207 | 判负 | <!-- gate-doc-ignore: 历史快照（消融实验实测值，非基线声明），⛔ 不改史 -->
| e18-pos20f10 | 4.68% | −3.90pp | 17.94% | +0.55pp | 4.65 | 247 | 判负 | <!-- gate-doc-ignore: 历史快照（消融实验实测值，非基线声明），⛔ 不改史 -->
| e18-pos20f10eq | 2.38% | −6.21pp | 3.62% | −13.77pp | 0.34 | 55 | 判负（退化为准指数买入持有：换手 0.34≈不调仓，MDD 极低但收益归零） | <!-- gate-doc-ignore: 历史快照（消融实验实测值，非基线声明），⛔ 不改史 -->

## 六、矩阵级裁决——**pos8 为孤峰，宽度轴关闭**

- **宽度-收益曲线（15 万本金、地板 2 万基线口径）**：
  pos3=4.02 → pos5=8.58 → **pos8=9.63（尖峰）** → pos10=5.74 →
  pos15=3.76 / pos20=4.68——尖峰两侧单调陡降，+25% 邻域（pos10）
  即崩 −2.85pp ⇒ e17-pos8 的 +1.05pp 属**孤点非平台**，不予晋级。
  <!-- gate-doc-ignore: 历史快照（消融实验宽度曲线实测值，非基线声明），⛔ 不改史 -->
- **机制解释（与 e16 带解剖 + D1 归因互洽）**：pos8 吃到的正好是
  dv 排序 r6-r8 这段「尚属良性」的候选——r9 起进入 e16 实测的
  逆向选择/陷阱带（r11-15 最差 −8.96pp），故 pos10 起急转直下；
  地板 10k 单独放行尾部小额仓位亦 −0.82pp ⇒ 2 万地板本身携带
  「剔除边缘票」的隐性过滤价值。
- **floor 效应结论**：`min_position_value` 不只是佣金地板，还是
  隐性的边缘仓位过滤器——对容量矩阵解读同样关键
  （15 万档 8.58% 含此红利，50 万+ 档无该过滤 ⇒ 见
  docs/CAPACITY_MATRIX_20260920.md 两阶段衰减解释）。
