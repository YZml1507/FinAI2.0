...
1. **e17-pos8（同批预登记臂）**：CAGR 9.63%（Δ=+1.05pp vs isst-e8b）， <!-- gate-doc-ignore: 历史快照（消融臂实测值引用-证据链，非基线声明），⛔ 不改史 -->
   MDD 15.17%（−2.23pp）——弱正证据带。 <!-- gate-doc-ignore: 历史快照（消融臂实测值引用-证据链，非基线声明），⛔ 不改史 -->
2. **e17-pos3**：4.02%（Δ=−4.57pp）——宽度轴单调性已现 <!-- gate-doc-ignore: 历史快照（消融臂实测值引用-证据链，非基线声明），⛔ 不改史 -->
   （pos3 < pos5 < pos8）。
