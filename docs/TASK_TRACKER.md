...
- **e7 行为探针已过（2026-09-17，终局级行为验证）**：`experiments/lab/e7-demote-audit/` 回放 2019-2021 on/off 双跑——flag_on 18 跨界日 0 违规（13 日 T+1 全清/4 日零市值/1 日部分成交疑停牌 warning）+ 时钟不变 + mid 持续期无重复下单 + ice 23 次正常；flag_off 同批日 15 日持仓 51-93% 带进警戒区（2020 失血归因的直接影像）。子窗口 flag_on +4.4pp。待 e7-demote 全窗口指标落地后按 PREREG 模板终裁。 <!-- gate-doc-ignore: 历史快照（探针回放实测值，非基线声明），⛔ 不改史 -->
