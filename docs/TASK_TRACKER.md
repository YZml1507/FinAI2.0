...
- **防御资产数据（已落盘）**：5 只 ETF 全历史入 `data/etf_bars/`（meta_defensive.json）：511360 短融 +2.45%/MDD-1.1%（价格口径真实✅）；511880/511990 货基价格口径失真（收益走份额结转，价格钉在~100）；511260 十年国债 +4.3%/MDD-4.6%；511220 疑数据/清盘异常（-90% MDD 待核）。Hermes 调研范围收窄为「规则/费率/逆回购利率序列」。
