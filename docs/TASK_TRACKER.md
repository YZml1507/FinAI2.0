...
> 创建：2026-09-15 17:45 ｜ 更新：2026-09-17 09:15（Alpha 三层终局：E4 rebalance 版 4.02%/MDD 37.8% → PEAD 判负关闭，全部构型低于基线，进入负证据固化收尾；36 新单测绿，基线 1064）｜ 更新规则：每完成一个小任务立即更新对应复选框与本节时间戳
...
- **E4 终局（已出）**：rebalance 版 CAGR 4.02%/MDD 37.78% < E2c 4.20% → **③PEAD 判负关闭**（两种建仓模式均无正贡献）；Alpha 三层全部构型低于基线 5.82%，按 PLAYBOOK 决策树进入诚实收尾：保底=宽度冠军构型+负证据文档化；E1 veto 的 MDD -1.8pp 防御价值留档备查。 <!-- gate-doc-ignore: 历史快照（消融实验实测值，非基线声明），⛔ 不改史 -->
