...
- **三组对照首批结果（2026-09-17 14:0x）**：`b-repro` CAGR/MDD 与冠军**逐值一致** ⇒ data_hash 漂移无实质影响（新增数据在回测窗外），旧基线继续有效，重锚完成 ✅；`e6-cash-yield-v3`（仅 2% 计息）**CAGR 7.27%/MDD 29.18%** vs 冠军——**计息有效且强于预期**（+1.45pp vs 预期 +0.48pp，因计息覆盖全部在账现金而非仅低仓日；MDD 降 2.1pp 系 NAV 水位抬升效应），终局登记有效，待 e6b 真实利率复核后晋级评估。`e7-demote` 首发失败（参数映射晚于发车进 commit）已重发；`e6b-gc001`（真实 GC001 序列，接口已实现：Ledger 双模式互斥+ffill≤5日断档 raise，commit 883bb38）并行在跑。 <!-- gate-doc-ignore: 历史快照（消融实验实测值，非基线声明），⛔ 不改史 -->
