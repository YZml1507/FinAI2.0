...
- **下一步（不等调研、不盲开实验）**：①审计 R8 T+1 对齐与参考层现金/损益处理，核对 e8b 是否已经对所有档位闲置现金计息；②校验指数日期顺序、代码映射及 ETF NAV/分拆口径；③复核门禁证据口径（逐年重置回测不等于连续 NAV 分年归因，价格基准不等于全收益基准）；④再据归因决定阈值稳健性机制研究或收益源重选，禁止加密已封顶网格、禁止放宽 G-2。
