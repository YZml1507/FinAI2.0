...
- 📊 **池依赖披露（2026-09-20）**：e22b 数据面补采取消——C3 管线已含退市票（缺口=0）；PIT 规则池对照=c3-p1-yearly（同参同日历）：ΔCAGR+0.66pp/ΔMDD+7.80pp 为池重选合成效应（不可分离幸存者单项，其唯一专项估计=R9-A −2.45pp/年影子）；对外口径「CAGR 8.6–9.2%/MDD 17–25%（池依赖）」，详见 FINAL_DELIVERY §十。

- 🧪 **e23 影子筛选预登记（2026-09-20 冻结，未运行）**：全 A 截面 6 非红利假设一次性筛选（docs/E23_SHADOW_SCREEN_PREREG.md），双闸+Bonferroni，仅「强」者进 e24 引擎预登记。

- 🧪 **e23 收单（2026-09-20）**：六假设全 A 影子筛选——无强者，e24 不触发；H2 反转「弱」（t=3.06，G4 暖机月失败）；项目趋向最终封板（E 路线工程 IP 打包继续）。
