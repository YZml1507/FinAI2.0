"""e59 I3 行业残差反转 z 日度序列预计算（e8b 叠加实验数据源）。

与 e36_c1_scores 同管线同 PIT 口径，唯一区别：只导出 I3 单信号
的日度截面 z（不做四族合成）。输出长表 parquet：

    data/i3_overlay/i3_scores_daily.parquet   (date, symbol, z)

用法：.venv/bin/python -m scripts.lab.e59_i3_scores --run [--limit-days N]
"""
...
OUT_PATH = ROOT / 'data' / 'i3_overlay' / 'i3_scores_daily.parquet'
...
def main() -> int:
    ...
    for i, T in enumerate(Ts):
        ...
        if s is None:
            continue
        z = _z(s).dropna()
        rows.extend((T.date().isoformat(), sym, float(v))
